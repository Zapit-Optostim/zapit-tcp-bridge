...
In this `README.md` we define the protocol, to allow users to implement their own clients in whatever language or context they wish, and additionally provide example clients in Python and in [Bonsai](https://bonsai-rx.org/).
The MATLAB TCP server and an example MATLAB client exists in the main zapit repo in `zapit\+zapit\+interfaces`.
We provide extensive documentation and demonstration scripts for all three languages in this repository.
